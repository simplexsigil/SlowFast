...
def color_to_depth(color_img: np.array, d_max, d_min, inverse_colorization=False):
    ...
    mask_sum_rgb_smaller_255 = color_img.sum(-1) < 255
    ...
    # if (r + g + b < 255) then dnormal = 0
    dnormal[mask_sum_rgb_smaller_255] = 0

    if inverse_colorization:
        disp_min = 1 / d_max
        disp_max = 1 / d_min
        d_recovery = disp_min + (disp_max - disp_min) * dnormal / 1529
        d_recovery = 1 / d_recovery + 0.5
    else:
        d_recovery = (d_min + (d_max - d_min) * dnormal / 1529) + 0.5

    d_recovery[mask_sum_rgb_smaller_255] = 0
...
if __name__ == '__main__':
    d_min, d_max = 1, 1529
    depth = np.arange(d_min, d_max)[np.newaxis, :].repeat(10, axis=0).astype(np.float32)
    # color_img = depth_to_color_realsense(depth, d_min=d_min, d_max=d_max, inverse_colorization=True)
    color_img = depth_to_color_corrected(depth, d_min=d_min, d_max=d_max, inverse_colorization=True)
    depth_rec = color_to_depth(color_img, d_min=d_min, d_max=d_max, inverse_colorization=True)

    assert np.equal(depth, depth_rec)
    print(1)
