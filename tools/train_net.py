...
from slowfast.models.contrastive import (contrastive_forward,
...
def train_epoch(train_loader,
                ...
                cur_epoch,
                cfg,
                writer: tb.TensorboardWriter = None, ):
    """
    ...
    if cfg.MIXUP.ENABLE:
        mixup_fn = MixUp(mixup_alpha=cfg.MIXUP.ALPHA,
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(train_loader):
        ...
        if writer is not None and cur_epoch == cfg.TRAIN.TBOARD_VIDEO_EPOCH and cur_iter == 0:
            vid_ins = inputs[0][:64]
            vid_ins = torch.einsum("bcthw -> bthwc", vid_ins).cpu()
            vid_ins = utils.revert_tensor_normalize(vid_ins, cfg.DATA.MEAN, cfg.DATA.STD)
            vid_ins = torch.einsum("bthwc -> btchw", vid_ins)

            for i, _ in enumerate(vid_ins):
                ln = f"{label_names[i]}-{label_names[::-1][i]}" if cfg.MIXUP.ENABLE else label_names[i]
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                (model,
                 preds,
                 partial_loss,
                 perform_backward,) = contrastive_forward(model, cfg, inputs, index, time, epoch_exact, scaler)
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if writer is not None:
                writer.add_scalars({"Train/loss": loss, "Train/lr": lr},
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                loss, grad_norm, top1_err, top5_err = (loss.item(),
                                                       grad_norm.item(),
                                                       0.0,
                                                       0.0,)
                if loss_extra:
            ...
            # Update and log stats.
            train_meter.update_stats(top1_err,
                                     top5_err,
                                     loss,
                                     lr,
                                     grad_norm,
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, train_loader, writer):
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.TASK == "ssl" and cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                if not cfg.CONTRASTIVE.KNN_ON:
                    return
                train_labels = (model.module.train_labels
                ...
                probs = torch.mul(retrieval_one_hot.view(batch_size, -1, C),
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                val_meter.iter_toc()
                # Update and log stats.
                val_meter.update_stats(top1_err,
                                       top5_err,
                                       batch_size
                                       * max(cfg.NUM_GPUS,
                                             1), )  # If running  on CPU (cfg.NUM_GPUS == 1), use 1 to represent 1 CPU.
        ...
        # write to tensorboard format if available.
        if writer is not None:
            writer.add_scalars({"Val/Top1_err": val_meter.mb_top1_err.get_win_median(),
...
def train(cfg):
    ...
    if cfg.TRAIN.AUTO_RESUME and cu.has_checkpoint(cfg.OUTPUT_DIR):
        ...
        if last_checkpoint is not None:
            checkpoint_epoch = cu.load_checkpoint(last_checkpoint,
                                                  model,
                                                  cfg.NUM_GPUS > 1,
                                                  optimizer,
        ...
        checkpoint_epoch = cu.load_checkpoint(cfg.TRAIN.CHECKPOINT_FILE_PATH,
                                              model,
                                              cfg.NUM_GPUS > 1,
                                              optimizer,
                                              scaler if cfg.TRAIN.MIXED_PRECISION else None,
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        is_checkp_epoch = (cu.is_checkpoint_epoch(cfg,
                                                  cur_epoch,
                                                  None if multigrid is None else multigrid.schedule, )
                           or cur_epoch == cfg.SOLVER.MAX_EPOCH - 1)
        is_eval_epoch = (misc.is_eval_epoch(cfg,
                                            cur_epoch,
                                            None if multigrid is None else multigrid.schedule, )
                         and not cfg.MASK.ENABLE)

        # Compute precise BN stats.
        if ((is_checkp_epoch or is_eval_epoch)
                and cfg.BN.USE_PRECISE_STATS
                and len(get_bn_modules(model)) > 0):
            calculate_and_update_precise_bn(precise_bn_loader,
                                            model,
        ...
        val_stats = {}
        if is_eval_epoch:
            val_stats = eval_epoch(val_loader,
                                   model,
                                   val_meter,
                                   cur_epoch,
                                   cfg,
    ...
    if writer is not None:
        writer.close()
    result_string = ("_p{:.2f}_f{:.2f} _t{:.2f}_m{:.2f} _a{:.2f} Top5 Acc: {:.2f} MEM: {:.2f} f: {:.4f}"
                     "".format(params / 1e6,
                               flops,
