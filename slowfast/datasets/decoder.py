...
def get_start_end_idx(
    ...
    """
    decode_boundaries = (
        [0, video_size] if decode_boundaries is None else decode_boundaries
    )

    # It is ok to decode with 25% overlap outside of segment, but make sure, to stay in video file.
    min_start, max_start = decode_boundaries[0] - clip_size // 4, decode_boundaries[
        1
    ] - 3 * (clip_size // 4)
    min_start, max_start = np.maximum(min_start, 0), np.minimum(
        max_start, video_size - clip_size
    )

    delta = max_start - min_start
    if clip_idx == -1:
        # Random temporal sampling.
        start_idx = random.uniform(min_start, max_start)
    else:
        if use_offset:
            if num_clips_uniform == 1:
                # Take the center clip if num_clips_uniform is 1.
                start_idx = np.maximum(
                    math.floor(min_start + delta / 2 - clip_size / 2), 0
                )
            else:
            ...
            # Uniformly sample the clip with the given index.
            start_idx = min_start + delta * clip_idx / num_clips_uniform

    start_idx = np.maximum(np.round(start_idx), 0.0)
    end_idx = np.round(start_idx) + clip_size - 1
...
def get_multiple_start_end_idx(
    ...
    def sample_clips(
        ...
        # It is ok to decode with 25% overlap outside of segment, but make sure, to stay in video file.
        min_starts, max_starts = decode_boundaries[
            0
        ] - clip_sizes // 4, decode_boundaries[1] - 3 * (clip_sizes // 4)
        min_starts, max_starts = np.maximum(
            min_starts, np.array([0] * len(clip_sizes))
        ), np.minimum(max_starts, video_size_all - clip_sizes)
                ...
                if clip_idx == -1:
                    ...
                    if use_offset:
                        if num_clips_uniform == 1:
                            ...
                            start_idx = min_start + math.floor(
                                (max_start - min_start) / 2
                            )
                        else:
                            start_idx = min_start + clip_idx * math.floor(
                        ...
                        start_idx = (
                            min_start
                            + (max_start - min_start) * clip_idx / num_clips_uniform
                        )
                start_idx = start_idx.astype(int)
                end_idx = start_idx.astype(int) + clip_size - 1
    ...
    if video_size_all is None:
        video_size_all = video_size  # Looking at the whole video instead of a segment.
...
def torchvision_decode(
    ...
    video_tensor = torch.from_numpy(
        np.array(np.frombuffer(video_handle, dtype=np.uint8), copy=True)
    )
    ...
    # For the moment we do not want partial decoding,
    # since we need the full number of frames to get
    # relative temporal position in the video TODO
    if (
        False
        and len(video_meta) > 0
        and (
        ...
        video_duration = (
            video_meta["video_duration"]
            if decode_boundaries is None
            else decode_boundaries[1] - decode_boundaries[0]
        )

        if not "video_duration" in video_meta:
            logger.warning(f"Could not extract video duration.")
        ...
        start_end_delta_time = get_multiple_start_end_idx(
            ...
            if decode_boundaries is not None
            else (0, video_meta["video_duration"] * video_meta["video_fps"]),
        )
    ...
    if expected_duration is not None and abs(expected_duration - all_duration) > 0.2:
        logger.warning(
            f"The actual length of the video file is off by {all_duration - expected_duration} seconds."
...
def pyav_decode(
    ...
    duration_sec = frames_length / fps

    if expected_duration is not None and abs(expected_duration - duration_sec) > 0.2:
        logger.warning(
            f"The actual length of the video file is off by {duration_sec - expected_duration} seconds."
            f"This might be a problem with label assignments and indicates wrong encoding/settings."
        )
...
def decode(
    ...
    if start_end_delta_time is None:
        # Todo: currently only works for single clip.
        start_end_delta_time = np.array([[start_idx, end_idx, 0]])

    assert (
        decode_all_video  # This we changed to make sure we can returnfull video length.
    )

    return frames_out, start_end_delta_time, time_diff_aug, frames_decoded[0].shape[0]
