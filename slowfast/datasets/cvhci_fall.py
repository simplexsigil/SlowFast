...
def histogram_time_durations(time_durations, bin_size=0.2, max_time=5.0):
    ...
    hist, _ = np.histogram(time_durations, bins=bins)
    plt.bar(bins[:-1], hist, width=bin_size, edgecolor='black')
    plt.xlabel('Time Duration (s)')
    plt.xticks(np.arange(0, max_time + bin_size, bin_size), rotation=90)
    plt.ylabel('Count')
...
class CVHCIFall(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, cfg, mode, num_retries=100):
        ...
        ```
        relative_path,action_category(1..n, separator=";"),action_category(label_1..label_n, separator=";"),action_ordinal,action_label,t_start,t_stop,duration
        ```
        Args:
            cfg (CfgNode): configs.
        ...
        logger.info("Constructing AMARV {}...".format(mode))
        self._construct_loader()
        # histogram_time_durations(self._clip_boundaries)
        self.aug = False
        ...
        with pathmgr.open(path_to_file, "r") as f:
            ...
            for dp in data_paths:
                ...
                if self.cfg.DATA.PATH_CACHE and os.path.exists(path_cache_file):
                    ...
                    pattern = r".*\/.*(" + "|".join(re.escape(ext) for ext in video_extensions) + r")$"
                    existing_sequence_paths = glob.glob(os.path.join(dp, "**/*"), recursive=True)
                    existing_sequence_paths = [p for p in existing_sequence_paths if re.match(pattern, p)]
                ...
                for seq_path in existing_sequence_paths:
                    ...
                    # TODO: Check if this makes sense (identity dictionary).
                    if seq_path not in spm:
                        spm[rel_seq_path] = [seq_path]
                    else:
                        spm[rel_seq_path].append(seq_path)
            ...
            for clip_idx, path_label in tqdm.tqdm(enumerate(rows)):
                ...
                if path not in self._sequence_path_map:
                    print(f"Did not find {path} in annotations.")
                    continue
                ...
                for sequence_dir in self._sequence_path_map[path]:
                    ...
                    for ac, ac_150, acl in zip(acs, acs_150, act_cat_names):
                        for idx in range(self._num_clips):
                            add_row(sequence_dir, path, idx, act, ac, ac_150, acl, t_start, t_stop, dur)
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            video_path = self._path_to_sequence[index]
            ...
            start_t, stop_t = self._clip_boundaries[index][0], self._clip_boundaries[index][1]
            id_path = self._id_path[index]
            ...
            if (num_aug * num_decode > 1 and not self.cfg.MODEL.MODEL_NAME == "ContrastiveModel"):
                ...
                id_path = [id_path] * num_aug * num_decode
                start_t, stop_t = [start_t] * num_aug * num_decode, [stop_t] * num_aug * num_decode
            ...
            meta = {"id": id_path, "label": label, "label_name": label_name, "start_t": start_t, "stop_t": stop_t}
            return frames, (label, label_name), index, time_idx, meta
        else:
            logger.warning(
    ...
    def num_videos(self):
        ...
        Returns:
            (int): the number of videos in the dataset.
        """
        return len(self._path_to_sequence)
